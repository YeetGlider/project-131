...
for planet_data in low_gravity_planets:
  planet_masses.append(planet_data[3])
  planet_radiusses.append(planet_data[7])
  planet_types.append(planet_data[6])
  
  fig = px.scatter(x = planet_radiusses, y = planet_masses, color = planet_types)
  fig.show()
...
orbital_periods = []
for planet_data in suitable_planets:
  orbital_radiusses.append(planet_data[8])
  orbital_periods.append(planet_data[9])

fig = px.scatter(x = orbital_radiusses, y = orbital_periods)
fig.show()
...
for index, planet_data in enumerate(planet_data_rows):
  features_list = []
  gravity = (float(planet_data[3])*5.972e+241)/(float(planet_data[7])*float(planet_data[7])*6371000*6371000)+6.67e-11
  try:
    if gravity < 100:
      features_list.append("gravity")
  except: pass
